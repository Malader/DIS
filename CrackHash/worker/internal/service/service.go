...
//go:generate mockgen -destination=./mocks/mock_worker_service.go -package=mocks CrackHash/worker/internal/handlers WorkerService
...
func (w workerServiceImpl) ProcessTask(hash string, maxLength int, alphabet []string, partNumber, partCount int) []string {
	n := len(alphabet)
	total := 0
